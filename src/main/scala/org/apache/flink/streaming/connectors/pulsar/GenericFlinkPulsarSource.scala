...
import org.apache.flink.streaming.connectors.pulsar.internal._
...
abstract class GenericFlinkPulsarSource[T](val parameters: Properties)
  ...
  var ownedTopicStarts: mutable.HashMap[String, MessageId] = _

  var periodicWatermarkAssigner:
    SerializedValue[AssignerWithPeriodicWatermarks[T]] = _

  var punctuatedWatermarkAssigner:
    SerializedValue[AssignerWithPunctuatedWatermarks[T]] = _
  ...
  @transient @volatile private var pulsarFetcher: PulsarFetcher[T] = _

  @transient @volatile private var topicDiscoverer: PulsarMetadataReader = _
   ...
   */
  @transient @volatile private var restoredState: TreeMap[String, MessageId] = _

  /** Accessor for state in the operator state backend. */
  @transient private var unionOffsetStates: ListState[Tuple2[String, MessageId]] = _

  /** Discovery loop, executed in a separate thread. */
  @transient @volatile private var discoveryLoopThread: Thread = _

  /** Flag indicating whether the reader is still running. */
  @volatile private var running: Boolean = true

  /** Counter for successful Pulsar offset commits. */
  @transient var successfulCommits: Counter = _

  /** Counter for failed Pulsar offset commits. */
  @transient var failedCommits: Counter = _

  /** Callback interface that will be invoked upon async pulsar commit completion. */
  private var offsetCommitCallback: PulsarCommitCallback = _
  ...
  def assignTimestampsAndWatermarks(
      assigner: AssignerWithPunctuatedWatermarks[T]): GenericFlinkPulsarSource[T] = {
    ...
    try {
      ...
      this.punctuatedWatermarkAssigner =
        new SerializedValue[AssignerWithPunctuatedWatermarks[T]](assigner)
      this
    } catch {
      case e: Exception =>
   ...
   */
  def assignTimestampsAndWatermarks(
      assigner: AssignerWithPeriodicWatermarks[T]): GenericFlinkPulsarSource[T] = {
    checkNotNull(assigner)
    ...
    try {
      ...
      this.periodicWatermarkAssigner =
        new SerializedValue[AssignerWithPeriodicWatermarks[T]](assigner)
      this
    } catch {
      case e: Exception =>
        throw new IllegalArgumentException("The given assigner is not serializable", e)
  ...
  override def initializeState(context: FunctionInitializationContext): Unit = {
    ...
    unionOffsetStates = stateStore.getUnionListState(
      new ListStateDescriptor[Tuple2[String, MessageId]](
        GenericFlinkPulsarSource.OFFSETS_STATE_NAME,
    ...
    if (!running) {
      ...
      if (fetcher == null) {
        ...
        while (pendingOffsetsToCommit.size() >
          GenericFlinkPulsarSource.MAX_NUM_PENDING_CHECKPOINTS) {
  ...
  protected def buildDeserializer(pulsarSchema: SchemaInfo,
                                  jsonOpts: JSONOptionsInRead): Deserializer[T]
  ...
  protected def createFetcher(
    ...
    streamingRuntime: StreamingRuntimeContext) : PulsarFetcher[T] = {
    new PulsarFetcher(
      sourceContext,
  ...
  protected def createTopicDiscoverer() : PulsarMetadataReader = {
...
object GenericFlinkPulsarSource {

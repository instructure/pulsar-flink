...
class PulsarFetcher[T](
  ...
  protected val subscribedTopicStates: ju.List[PulsarTopicState] =
  ...
  def runFetchLoop(): Unit = {
    ...
    val isPoison = new Predicate[PulsarTopicState] {
      override def test(t: PulsarTopicState): Boolean = PoisonState == t
    }
    ...
    try {
      ...
      while (running) {
        ...
        topicsToAssign.removeIf(isPoison)
  ...
  def removeEarliestAndLatest(offset: Map[String, MessageId]): Map[String, MessageId] = {
    offset.filterNot { case (_, mid) =>
      mid == MessageId.earliest || mid == MessageId.latest
    }
  }

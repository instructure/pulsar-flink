 ...
 *
 */
class PulsarTopicStateWithPeriodicWatermarks[T](
    topic: String,
    periodicWatermarks: AssignerWithPeriodicWatermarks[T])
  ...
  def getTimestampForRecord(record: T, timestamp: Long): Long = {
    periodicWatermarks.extractTimestamp(record, timestamp)
  }
 ...
 *
 */
class PulsarTopicStateWithPunctuatedWatermarks[T](
    topic: String,
    punctuatedWatermarks: AssignerWithPunctuatedWatermarks[T])
  extends PulsarTopicState(topic) {

  /** The last watermark timestamp generated by this partition. */
  @volatile var partitionWatermark: Long = Long.MinValue

  def getTimestampForRecord(record: T, pulsarEventTimestamp: Long): Long = {
    punctuatedWatermarks.extractTimestamp(record, pulsarEventTimestamp)
  }

  def checkAndGetNewWatermark(record: T, timestamp: Long): Watermark = {
    val wm = punctuatedWatermarks.checkAndGetNextWatermark(record, timestamp)
